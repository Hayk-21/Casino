    ...
    public function handle(GamePlayed $event)
    {
        ...
        if ($user->referrer) {
            ...
            if ($loss > 0 && $lossBonusPct > 0) {
                BonusServiceExtend::create($user->referrer->account, $user->id, Bonus::TYPE_REFERRER_GAME_LOSS, $loss * $lossBonusPct / 100);
            // win
            } else if ($win > 0 && $winBonusPct > 0) {
                BonusServiceExtend::create($user->referrer->account, $user->id, Bonus::TYPE_REFERRER_GAME_WIN, $win * $winBonusPct / 100);
            }
        }
    }
}
