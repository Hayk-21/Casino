...
@section('content')
    ...
    @elseif(isset($payment['time_created']) && isset($payment['time_expires']) && isset($payment['payment_address']))
        @if($payment['time_expires'] > time())
            ...
            @if($deposit->destination_tag)
                <div class="form-row mt-1">
                    <div class="form-group col">
                        <label>{{ __('Destination tag') }} / {{ __('Memo') }}</label>
                        <div class="input-group">
                            <input id="destination-tag-input" type="text" class="form-control text-muted" value="{{ $deposit->destination_tag }}" readonly>
                            <div class="input-group-append">
                                <button type="button" class="btn btn-primary" onclick="copyToClipboard(document.getElementById('destination-tag-input'))"><i class="far fa-copy"></i> {{ __('Copy') }}</button>
                            </div>
                        </div>
                    </div>
                </div>
            @endif
            <p class="mt-1">{{ __('Time left') }}:</p>
            <div class="progress">
