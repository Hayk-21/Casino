...
<table class="table table-hover table-stackable">
	...
	<tbody>
		@foreach (s as )
		<tr>
			<td data-title="{{ __('ID') }}">{{ ->id }}</td>
			<td data-title="{{ __('Winner') }}">
				<a href="{{ route('frontend.users.show', ->winner) }}">{{ ->winner->name }}</a>
			</td>
			<td data-title="{{ __('Win amount') }}" class="text-right">{{ ->_win_amount }}</td>
			<td data-title="{{ __('Win amount') }}" class="text-right">
				{{ ->tickets->count() }} / {{ ->total_tickets }}
			</td>
			<td data-title="{{ __('Created') }}" class="text-right">
				{{ ->end_date->diffForHumans() }}
				<span data-balloon="{{ ->end_date }}" data-balloon-pos="up">
