...
class GameMultiSlotsService extends GameService
{
    ...
    public function __construct($user = NULL)
    {
        ...
        $titles = config('game-multi-slots.titles');
        $slugs = config('game-multi-slots.slugs');
        $index = request()->route()->index ?? NULL;

        // game is accessed by slug
        if (!is_null($index) && preg_match('#[a-z]+#i', $index)) {
            $i = array_search($index, $slugs, TRUE);
            $this->index = $i !== FALSE ? $i : NULL;
        // game is accessed by its index
        } else if (!is_null($index) && $index == intval($index) && array_key_exists($index, $titles)) {
            $this->index = $index;
        // randomly choose slots game index when it is created from console and HTTP request is not available
        } elseif (is_null($index)) {
            $this->index = random_int(0, count($titles)-1);
        }
